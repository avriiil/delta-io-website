...
Parquet files are immutable, so you can’t modify the file to update the column name. If you want to change the column name, you will have to read the file into a DataFrame, change the column name, and then rewrite the entire file. Renaming a column can be an expensive computation.

Delta Lake abstracts the concept of physical column names and logical column names. The physical column name is the actual column name in the Parquet file. The logical column name is the column name humans use when referencing the column.
